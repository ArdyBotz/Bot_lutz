...
const reply = (text) => {
  m.reply(text);
    ...
    // Ambil 10 digit terakhir dari nomor pengguna
    const userIdDigits = userNumber.replace(/\D/g, '').slice(-10);
    const excelPath = path.join(excelFolderPath, `${userIdDigits}.xlsx`);
    ...
    try {
        ...
        console.error("Error membaca database orders.json:", e);
        return m.reply("⚠️ Terjadi kesalahan saat membaca database!");
    }
    ...
    for (let user in ordersDB.users) {
        ...
        if (orderIndex !== -1) {
            ...
            // Format ulang harga ke format Rupiah
            ...
            pesan += `💰 *Total Harga*: ${formatRupiah(userOrders[orderIndex].harga)}\n`;
            ...
            try {
                ...
                console.error("Error saat menulis ke database:", e);
                return m.reply("⚠️ Terjadi kesalahan saat menyimpan perubahan!");
            }
    ...
    if (body.trim() === ".input") {
        m.reply(
            ...
            "WA: 081234567890\n" +
            "```" +
            "\n\n⏳ Kami akan mengirim format yang dapat Anda isi, mohon tunggu..."
        );
    ...
    let data = {
        ...
        harga: parseHarga((body.match(/Harga:\s*(.+)/i) || [])[1]),
    ...
    // Cek apakah file database saldo ada
    if (fs.existsSync(saldoDbPath)) {
        fs.writeFileSync(saldoDbPath, "[]"); // Mengosongkan file dengan format array kosong
        return m.reply("✅ *Database saldo berhasil direset!*");
    } else {
        return m.reply("⚠️ *Database saldo belum pernah dibuat atau sudah kosong.*");
    }
}
  ...
  try {
    ...
    if (productData && productData.length > 0) {
      ...
      // **Pengecekan Produk E-Wallet dan Pembulatan Harga**
      const eWalletBrands = ["DANA", "GOPAY", "OVO", "SHOPEEPAY", "LINKAJA"];
      if (eWalletBrands.includes(product.brand.toUpperCase())) {
        hargaWithPercentage = Math.floor(hargaWithPercentage / 1000) * 1000;
      }
      ...
      const topData = [
        {
          ...
          harga: hargaWithPercentage, // Simpan harga yang sudah dihitung
        },
      ];
      fs.writeFileSync(
      ...
      return m.reply(
        `Layanan ${kodeproduk} tidak tersedia\nSilahkan pilih layanan yang lain`
      );
    }
  } catch (error) {
  ...
  try {
    ...
    if (productData && productData.length > 0) {
      ...
      const hargaGold = formattedBalance(product.price * gold);
      let seller = product.seller_product_status ? "Tersedia" : "Tidak Tersedia";
      let buyer = product.buyer_product_status ? "Tersedia" : "Tidak Tersedia";
      ...
      const capt = `──〔 DETAIL PRODUCT 〕──\n\n*» Item :* ${
        ...
        product.category
      }\n*» Status Seller :* ${seller}\n*» Status Buyer :* ${buyer}\n\nJika Punya Pertanyaan Hubungi Kami. Terima Kasih :)`;
